...
class Geom {
    ...
    static StaminaSeedB() => Round(0.0202 * this.ch + 12)
...
class Rgn {
    ...
    static Stamina(B := 0) => Geom.BL(21, B ? B : Read.staminaB, 268, 1)

    static AutoWeight() => Geom.BL(316, 10, 80, 24)

    static Phase() => Geom.BL(120, 214, 60, 16)

    static MenuX() => Geom.C(263 - 4, -112.5 - 4, 11 + 8, 12 + 8)

    static MenuPanel() => Geom.C(-302, -141.5, 604, 341)

    static Reconnect() => Geom.C(30, 79, 125, 16)

    static DialogBg()    => Geom.C(-190, 25, 380, 30)
    static LeaveButton() => Geom.C(-175, 72, 160, 30)
...
class Diag {
    static Report() {
        ...
        s .= "  row lock: " (Read.lockedAt ? Read.lockedAt : "not locked yet") "`n`n"
...
class Update {
    ...
    static Newer(a, b) {
        ...
        loop 3 {
            x := (pa.Length >= A_Index) ? Integer(RegExReplace(pa[A_Index], "\D")) : 0
            y := (pb.Length >= A_Index) ? Integer(RegExReplace(pb[A_Index], "\D")) : 0
            if (x != y)
                return x > y
        }
        return false
        ...
        switch grp {
            ...
            case "type":
                ...
                if (fid = "pick")
                    this.picking := true
                this.BeginEdit(fid, "")
            case "keys":  this.showKeys := !this.showKeys
...
class Overlay {
    ...
    static Watched() => Map(
        "STAMINA", Rgn.Stamina(),
        "PHASE",   Digits.PhaseRegion(),
        "WEIGHT",  Digits.WeightRegion(),
        "LIFTING", Rgn.AutoWeight(),
        "MENU",    Rgn.MenuX())
