...
;  SPEARHEADS  ·  Ultimate Gym Game Macro  ·  v3.0.0
...
class CFG {
    ...
    static NOREAD_REP_MS      := 900
    static NOREAD_REGEN_SINGLE := 2600
    static NOREAD_REGEN_FULL   := 6500
    ...
    static REP_RETRY_MS      := 90

    static REP_RETRIES       := 15
...
class Rd {
    ...
    static On(k) => this.FLAGS.Has(k) ? this.FLAGS[k] : true
    static Set(k, v) {
        this.FLAGS[k] := v
        TestCfg.Save()
        UI.dirty := true
...
class Rgn {
    ...
    static AutoWeight() =>
        Geom.BL(Calib.S(316), Calib.S(10), Calib.S(80), Calib.S(24))
...
class Calib {
    ...
    static ScanRow(bFromBottom, maxX) {
        r := Geom.BL(0, bFromBottom, maxX, 1)
        if !(frame := Cap.Grab(r))
            return { len: 0, start: 0 }
        best := 0, bestStart := 0, run := 0, start := 0
        loop maxX {
            i := A_Index - 1
            v := Cap.Px(frame, i)
            lit := Max(Cap.R(v), Max(Cap.G(v), Cap.B(v))) >= CFG.PX_EMPTY_MAX
            if lit {
                if !run
                    start := i
                run++
    ...
    static Run() {
        ...
        maxX := Min(Geom.cw - 1, Round(this.BASE_W * CFG.CALIB_MAX_SCALE) + 120)
        bestLen := 0, bestB := 0, bestStart := 0

        b := CFG.CALIB_B_MIN
        while (b <= CFG.CALIB_B_MAX) {
            s := this.ScanRow(b, maxX)
            if (s.len >= this.BASE_W * 0.8 && s.start <= Round(60 * CFG.CALIB_MAX_SCALE)) {

                if (s.len > bestLen * 1.02 || !bestLen) {
                    bestLen := s.len, bestB := b, bestStart := s.start
                    break
                }
            }
            b++
        ...
        if !bestLen {
            this.why := "could not find the stamina bar -- stand still with a full bar"
            return false
        }

        top := bestB, bot := bestB
        while (bot > CFG.CALIB_B_MIN - 6) {
            s := this.ScanRow(bot - 1, maxX)
            if (s.len < bestLen * 0.85)
                break
            bot--
        }
        while (top < CFG.CALIB_B_MAX + 30) {
            s := this.ScanRow(top + 1, maxX)
            if (s.len < bestLen * 0.85)
                break
            top++, bestLen := Max(bestLen, s.len)
        }

        this.barL := bestStart, this.barW := bestLen
        this.barBot := bot, this.barTop := top
        this.scale := Round(bestLen / this.BASE_W, 3)
        if (this.scale < 0.6 || this.scale > CFG.CALIB_MAX_SCALE) {
            this.why := "measured scale " this.scale "x is out of range"
            return false
        }

        this.rowB := bot + Max(2, Round(4 * this.scale))
        this.ok := true, this.why := ""
        Log.Add(Format("calibrated: bar {1}px at L={2}, B {3}..{4} -> scale {5}x, row B={6}",
                       this.barW, this.barL, bot, top, this.scale, this.rowB))
        return true
    }

    static S(px) => Round(px * this.scale)
    static Desc() => this.ok
...
class Read {
    ...
    static LockStaminaRow() {
        ...
        B := seed - CFG.STAM_SEARCH_PX
        while (B <= seed + CFG.STAM_SEARCH_PX) {
            st := this.Stamina(B)
    ...
    static AutoWeight() {
        ...
        loop total {
            v := Cap.Px(frame, A_Index - 1)
            if this._Near(v, 0x8A3E3E, 22)
                offN++
            else if this._Near(v, 0x34764C, 22)
                onN++
        }
        best := Max(offN, onN)
        if (best < total * 0.45)
...
class Digits {
    ...
    static PhaseRegion()  => Geom.BL(112, 206, 200, 29)
    static WeightRegion() => Geom.BR(300, 425, 199, 49)
...
class Engine {
    ...
    static S_Gate() {
        if !Rd.On("stamina") {
            ...
            need := MODES.IsSingle(this.mode) ? CFG.NOREAD_REGEN_SINGLE
                                              : CFG.NOREAD_REGEN_FULL
    ...
    static S_AwaitLift() {
        ...
        if this.Wait(CFG.LIFT_WAIT_MS) {
            ...
            if (this.target.hasMenu && Rd.On("menu")) {
                m := Read.MenuOpen()
                if (m.ok && m.open && this.retries < CFG.PROMPT_RETRIES) {
                    this.retries++
                    Log.Add("menu still open after Enter -- re-committing")
                    this.navSent := 0
                    this.Go("NAV_ROW")
                    return
                }
    ...
    static S_RepRead() {
        if !Rd.On("stamina") {
            ...
            if !this.Wait(CFG.NOREAD_REP_MS)
                return
            this.reps++
            this.repTries := 0
            Stats.AddRep(this.target.name)
            this.status := Format("{1} - rep {2} (timed)", this.target.name, this.reps)
        ...
        if (after < 0 || (before - after) < CFG.REP_MIN_DELTA) {
            ...
            this.deadline := A_TickCount + CFG.REP_RETRY_MS
            this.Go("REP")
            return
        }
    ...
    static S_PostRep() {
        ...
        nominal := MODES.NOMINAL.Has(this.mode) ? MODES.NOMINAL[this.mode] : 1
        this.Go(this.reps >= nominal ? "EXIT_SPACE" : "REP")
    }
...
class Diag {
    static Report() {
        ...
        aw := Read.AutoWeight()
        s .= aw.ok
            ? Format("  autoweight`tlifting={1}  on={2}  (off {3}px / on {4}px)`n",
        ...
        s .= "  disconnect`t" (Read.Disconnected() ? "DIALOG PRESENT" : "no") "`n`n"
    ...
    static _LiveTick() {
        ...
        mo := Read.MenuOpen()
        ToolTip st.ok
            ? Format("stamina {1}%  f{2}/e{3}/l{4}  row B={5}`nlifting {6}  autoweight {7}  menu {8}",
                     Round(st.pct * 100), st.filled, st.empty, st.label, Read.staminaB,
                     aw.lifting, aw.on, mo.open)
...
class Shaker {
    ...
    static Start() {
        ...
        this.running := true, this.idx := 0, this.walked := 0, this.navOn := false
    ...
    static S_Walk() {
        ...
        need := this.idx
        if (this.walked >= need || this.walked >= CFG.SHK_SLOTS - 1) {
        ...
        if (this.deadline && A_TickCount < this.deadline)
            return
        this.status := Format("Shaker: slot {1} of {2}", this.walked + 2, CFG.SHK_SLOTS)
        if !Inp.Key(CFG.KEY_RIGHT)
            return
        this.walked++
    ...
    static S_Pick() {
        if !this.seq.Length {
            ...
            this.seqIdx := 0
            this.status := Format("Shaker: scoop {1}/{2} into slot {3}",
                                  this.idx + 1, this.recipe.Length, this.idx + 1)
        }
    ...
    static S_Final() {
        if !this.seq.Length {
            ...
            this.seq := ["", CFG.KEY_RIGHT, CFG.KEY_ENTER]
            this.seqIdx := 0
        }
        if (this.seqIdx = 0) {
            if (this.deadline && A_TickCount < this.deadline)
        ...
        switch grp {
            ...
            case "dbg":
                if (item = "outline")
                    ...
                    Read.InvalidateLock()
                    if (Calib.Run() && Read.LockStaminaRow())
...
class Overlay {
    ...
    static Tick() {
        ...
        manual := (this.until && A_TickCount < this.until)
...
class Paint {
    ...
    static TabLifting(g, x, y, w) {
        ...
        Gfx.Text(g, okRead ? Round(pct * 100) "%" : "no read",
                 x + w - 60, y, 60, UX.ROW, "value", 2, okRead ? 0 : UX.WARN)
        this.Bar(g, x + w * 0.3, y + 12, w * 0.7 - 68, UX.ROW - 24, pct,
        ...
        switch UI.sub {
            case "GENERAL":
                ...
                if UI.showReaders {
                    ...
                    for , rk in Rd.ORDER {
                        ...
                        Gfx.Text(g, on ? "" : Rd.Cost(rk), x + w * 0.4, y,
                                 w * 0.6 - 60, UX.ROW - 8, "dim", 0, UX.WARN)
                        bw := 52, bx := x + w - bw
                        Gfx.Rect(g, bx, y + 4, bw, UX.ROW - 16,
                                 on ? UX.ACCENT : UX.PANEL_HI)
                        Gfx.Rect(g, bx, y + 4, bw, 2, on ? UX.ACCENT_HI : UX.EDGE)
                        Gfx.TrackedC(g, on ? "ON" : "OFF", bx, y + 4, bw, UX.ROW - 16,
                                     "dim", 2, on ? UX.BG : UX.DIM)
            ...
            case "DEBUG":
                ...
                this.Btn(g, "dbg:calib", x, y + 3, half, UX.ROW - 8,
                         "RE-SCAN STAMINA BAR")
                this.Row(g, x + half + UX.S3, y, half, "HUD SCALE", Calib.Desc(),
                         Calib.ok ? 0 : UX.WARN)
                y += UX.ROW
...
OnExit((*) => (Overlay.Close(), UI.Close(), Cap.Release(), 0))
